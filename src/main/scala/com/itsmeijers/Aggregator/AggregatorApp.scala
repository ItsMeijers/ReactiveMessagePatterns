package com.itsmeijers.Aggregator

import com.itsmeijers.CompletableApp
import akka.actor._

class AggregatorApp extends CompletableApp(5) {
   ...
   val priceQuoteAggregator =
      system.actorOf(Props(classOf[PriceQuoteAggregator]), "priceQuoteAggregator")

   val orderProcessor = system.actorOf(
      Props(classOf[MontaineeringSuppliesOrderProcessor], priceQuoteAggregator, this),
      "orderProcessor")
...
class MontaineeringSuppliesOrderProcessor(
   ...
   def receive = {
      ...
      case rfq: RequestForQuotation =>
         ...
         priceQuoteAggregator ! RequiredPriceQuotesForFulfillment(
                                    rfq.rfqId,
                                    recipientList.size * rfq.retailItems.size)
...
class PriceQuoteAggregator extends Actor {
   val fulfilledPriceQuotes = mutable.Map[String, QuotationFulfillment]()

   def receive = {
      case required: RequiredPriceQuotesForFulfillment =>
         fulfilledPriceQuotes(required.rfqId) = QuotationFulfillment(
                                                   required.rfqId,
      ...
      case priceQuoteFulfilled: PriceQuoteFulfilled =>
         ...
         val currentFulfillment = QuotationFulfillment(
      ...
      case msg: Any =>
         println(s"PriceQuoteAggregator: unexepected: $msg")
   }
}
